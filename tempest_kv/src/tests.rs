...
fn test_storage_basic() {
    ...
    tokio_uring::start(async {
        ...
        {
            // initialize storage
            let mut storage: Storage<_, DefaultComparer> =
            ...
            info!("first silo after shutdown: {:#?}", storage);
        }

        {
            ...
            for &(k, v) in &kvs {
                let found = storage
                    .get(&k.into(), storage.highest_seqnum())
                    .await
                    .unwrap();
                assert_eq!(
                    found.as_deref(),
            ...
            // shut down silo
            storage.shutdown().await.unwrap();
            info!("second silo after shutdown: {:#?}", storage);
        }
    });
}

#[test]
fn test_storage_scan_interleaving_and_deduplication() {
    setup_tracing();

    let id = 1;
    let storage_dir = "/silo-scan-test";
    let fs = VirtualFileSystem::new();
...
fn test_storage_load() {
    ...
    let id = 1;
    let storage_dir = "/silo-load-test";
    let fs = VirtualFileSystem::new();
    ...
    tokio_uring::start(async {
        ...
        for i in 0..batch_count {
            ...
            silo.write(batch).await.unwrap();

            if i % restart_interval == 0 && !is_first_or_last {
                restart_counter += 1;
                silo.shutdown().await.unwrap();
                silo = Storage::<_, DefaultComparer>::init(
                    id,
                    fs.clone(),
                    storage_dir,
                    config.clone(),
        ...
        for i in 0..batch_count {
            let key_b = format!("keyB:{}", i % override_wrap);
            let found = silo.get(&Bytes::from(key_b), snapshot).await.unwrap();
            ...
            let key_a = format!("keyA:{}", i % override_wrap);
            let found_a = silo.get(&Bytes::from(key_a), snapshot).await.unwrap();
...
#[test]
fn test_storage_recovery_seqnum_visibility() {
    ...
    setup_tracing();
    let id = 2;
    let storage_dir = "/silo-seqnum-visibility";
    let fs = VirtualFileSystem::new();
    ...
    tokio_uring::start(async {
        ...
        // -- Phase 3: reopen and verify the unflushed entries are visible --
        {
            ...
            for key in &["after:0", "after:1", "after:2"] {
                let found = silo
                    .get(&Bytes::from(key.to_string()), snapshot)
                    .await
                    .unwrap();
                assert!(
                    found.is_some(),
